...
if ($eventGridEvent) {
    ...
    $resourceManagedIdentityName = $resource.ResourceGroupName.Replace('RG', 'MI') + '-SqlSrvManagedInstance'
    try {
    ...
    if ($managedIdentityExists -eq $false) {
        ...
        try {
            ...
            Set-AzSqlInstance @paramsAssignManagedIdentity
        }
        catch {
            $paramsDatabaseRecord.exception = $_
            Write-ToDatabase @paramsDatabaseRecord
    ...
    try {
        ...
        # Note: I couldn't get this to work with Set-AzSqlInstanceTransparentDataEncryptionProtector.
        ...
        Set-AzSqlInstance @paramsUseTdeKey
    }
    catch {
        $paramsDatabaseRecord.exception = $_
        Write-ToDatabase @paramsDatabaseRecord
