...
if ($eventGridEvent) {
    ...
    try {
        ...
        # $SA = Get-AzStorageAccount -Name $storageAccount.Name -ResourceGroupName $storageAccount.ResourceGroupName
    }
    catch {
    ...
    try {
        $storageAccount.Properties.isSftpEnabled = $disableSftp
