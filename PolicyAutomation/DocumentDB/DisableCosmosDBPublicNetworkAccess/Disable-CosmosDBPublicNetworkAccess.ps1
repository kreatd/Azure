...
#constants:
$remediationDeploymentName = 'cosmosDBAccountDisallowPublicNetworkAccess'

# mandatory configurations:
$disablePublicNetworkAccess = 'Disabled'
...
if ($eventGridEvent) {
    ...
    try {
        $paramsGetCosmosDBAccount = @{
            ResourceId = $resourceId
            ExpandProperties = $true
        }
        $cosmosDBAccount = Get-AzResource @paramsGetCosmosDBAccount
    }
    catch {
    ...
    try {
        $cosmosDBAccount.Properties.publicnetworkaccess = $disablePublicNetworkAccess
        $cosmosDBAccount | Set-AzResource -Force
    }
    catch {
